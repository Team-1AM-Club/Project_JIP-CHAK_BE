...
_security = _load_first_json(
    DATA_DIR / "치안리스크" / "meta_security_stats_updated.json",
    DATA_DIR / "치안리스크" / "meta_security_stats.json",
)
...
_health = _load_first_json(
    DATA_DIR / "의료접근성" / "meta_health_unified_stats_updated.json",
    DATA_DIR / "의료접근성" / "meta_health_unified_stats.json",
    DATA_DIR / "의료접근성" / "meta_health_stats.json",
)
_congestion = _load_first_json(
    DATA_DIR / "생활혼잡도" / "meta_congestion_stats_updated.json",
    DATA_DIR / "생활혼잡도" / "meta_congestion_stats.json",
)

META_STATS: dict[str, dict[str, float]] = {
    **_stat_items(_security),
    **_stat_items(_flood.get("stats", _flood)),
